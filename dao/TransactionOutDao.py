...
def save(txOut, tx, index):
    ...
    if TransactionUtils.isCFTransation(tx):
        if 0 == index:
            ...
            useSate = 2
    CoinSqlite3().exec_sql('INSERT INTO TransactionInfoOut(coin_value, script, parentBlockId, parentTxId, state, `index`, pubicAddress, isToMe, usedState, end_time) VALUES (?,?,?,?,?,?,?,?,?,?)', txOut.coin_value, txOut.script, tx.getBlockHash(), tx.hash(), txOut.state, index, pubicAddress, SecretKeyDao.isMypubicAddress(pubicAddress), useSate, end_time)
